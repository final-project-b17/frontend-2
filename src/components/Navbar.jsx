...
const Navbar = () => {
   ...
   const Menus = [
      { name: "Kelas", icon: <TfiMenuAlt className="w-8 h-6" />, link: "/course", dis: "translate-x-[2px]" },
      { name: "Notifikasi", icon: <IoNotifications className="w-8 h-6" />, link: "/notif", dis: "translate-x-[84px]" },
      { name: "Akun", icon: <FaUser className="w-8 h-6" />, link: "/user", dis: "translate-x-[165.5px]" },
   ]
      ...
      <>
         <nav className="w-full">
            <div className="flex justify-between bg-indigo-600 px-2 py-4 lg:pt-6 lg:px-10 items-center">
               ...
               <div>
                  <img
                     src="../../src/assets/no bg putih.png" alt="logo"
                     className="w-10 h-10" />
               </div>
               ...
               <div className="md:flex md:flex-row-reverse md:gap-8 md:items-center">
                  ...
                  <div className="relative flex gap-4 lg:hidden">
                     <div className="md:hidden">
                        ...
                        <div className={`${isSearchOpen ? '-translate-x-28' : '-translate-x-[700px]'}  transition-transform duration-500 ease-in-out absolute top-0 right-0 flex -mt-1`}>
                           <input
                              type="text"
                     ...
                     <div className="relative">
                        ...
                        <div className={`${openHamburger ? 'translate-y-0' : '-translate-y-72'} transition-transform duration-300 ease-in-out absolute top-0 right-0 mt-16  bg-gradient-to-l from-indigo-200 border border-indigo-300 p-4 rounded-md shadow-lg `}>
                           <ul className="flex flex-col gap-4">
                              <NavLink as={Link} to={'/'}>
                                 <li onClick={() => handleIconClick('Masuk')} className="flex flex-row-reverse">
                                    {selectedIcon === 'Masuk' ? (
                              ...
                              <NavLink as={Link} to={'/course'}>
                                 <li onClick={() => handleIconClick('Kelas')} className="flex flex-row-reverse">
                                    {selectedIcon === 'Kelas' ? (
                                       <div className="flex flex-row-reverse gap-2  bg-indigo-600 py-2 px-3 rounded-lg text-white">
                              ...
                              <NavLink as={Link} to={'/notif'}>
                                 <li onClick={() => handleIconClick('Notifikasi')} className="flex flex-row-reverse">
                              ...
                              <NavLink as={Link} to={'/user'}>
                                 <li onClick={() => handleIconClick('Akun')} className="flex flex-row-reverse">
                                    {selectedIcon === 'Akun' ? (
                                       <div className="flex flex-row-reverse gap-2  bg-indigo-600 py-2 px-3 rounded-lg text-white">
                  ...
                  <div className="hidden md:block lg:block">
                     <form action="search" className="font-poppins flex">
                        <input
                           ...
                           name="search"
                        />
                        <button type="submit" className=" bg-white rounded-e-2xl flex items-center px-2">
               ...
               <div className="hidden items-center lg:flex">
                  ...
                  <ul className="flex relative gap-6 items-center">
                     <span className={`bg-white duration-500 ${Menus[active].dis} border-2 border-white h-14 w-14 absolute -left-2 top-2 rounded-t-full`}></span>
                     {
                        Menus.map((menu, i) => (
                           <li key={i}>
                              {/* {acttive} */}
                              <NavLink
                                 ...
                                 onClick={() => setActive(i)}
                              >
                                 <span className={`z-10 duration-500 text-white hover:text-slate-400 ${i === active && "-mb-12 text-yellow-400"}`}>
                                    {menu.icon}
                                 </span>
                                 <p className={`text-white font-poppins font-medium ${active === i
